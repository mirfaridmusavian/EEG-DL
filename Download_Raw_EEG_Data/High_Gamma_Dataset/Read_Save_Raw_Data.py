...
if __name__ == '__main__':
    ...
    train_set, valid_set, test_set = load_train_valid_test(train_filename=train_filename,
                                                           ...
                                                           debug=False)
    training_set = np.array(train_set.X)
    training_set = np.swapaxes(training_set, 1, 0)
    training_set = np.reshape(training_set, [np.shape(training_set)[0], np.shape(training_set)[1]*np.shape(training_set)[2]])
    training_set = np.transpose(training_set)

    training_label = np.array(train_set.y)
    training_label = np.squeeze(training_label)
    training_label = np.tile(training_label, [np.shape(train_set.X)[2], 1])
    training_label = np.transpose(training_label)
    training_label = np.reshape(training_label, [np.shape(training_set)[0], 1])

    validation_set = np.array(valid_set.X)
    validation_set = np.swapaxes(validation_set, 1, 0)
    validation_set = np.reshape(validation_set, [np.shape(validation_set)[0], np.shape(validation_set)[1]*np.shape(validation_set)[2]])
    validation_set = np.transpose(validation_set)

    validation_label = np.array(valid_set.y)
    validation_label = np.squeeze(validation_label)
    validation_label = np.tile(validation_label, [np.shape(valid_set.X)[2], 1])
    validation_label = np.transpose(validation_label)
    validation_label = np.reshape(validation_label, [np.shape(validation_set)[0], 1])

    testing_set = np.array(test_set.X)
    testing_set = np.swapaxes(testing_set, 1, 0)
    testing_set = np.reshape(testing_set, [np.shape(testing_set)[0], np.shape(testing_set)[1]*np.shape(testing_set)[2]])
    testing_set = np.transpose(testing_set)

    testing_label = np.array(test_set.y)
    testing_label = np.squeeze(testing_label)
    testing_label = np.tile(testing_label, [np.shape(test_set.X)[2], 1])
    testing_label = np.transpose(testing_label)
    testing_label = np.reshape(testing_label, [np.shape(testing_set)[0], 1])

    Dataset = np.row_stack((training_set, validation_set, testing_set))
    Labels  = np.row_stack((training_label, validation_label, testing_label))

    np.savetxt(SAVE + "Dataset.csv", Dataset, delimiter=",")
    np.savetxt(SAVE + "Labels.csv", Labels, delimiter=",")
